        ...
        <div className="flex items-center gap-2 justify-end">
          {status !== "Paid" && (
            <Link
              ...
              className="inline-block py-1.5 px-3.5 bg-blue-600 hover:bg-blue-700 text-white border-none rounded-[7px] text-[0.8rem] font-semibold no-underline transition-colors whitespace-nowrap"
            >
              Pay Now
            </Link>
          )}
            ...
            <>
              {receiptUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-block py-[5px] px-3 bg-transparent hover:bg-blue-50 text-blue-600 border border-blue-600 rounded-[7px] text-[0.8rem] font-semibold no-underline transition-colors whitespace-nowrap"
                >
                  View Receipt
                </a>
              ) : (
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
        ...
        <p className="mt-4 text-[0.9rem] text-gray-500 font-medium">Loading payments…</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-8 px-6 font-sans text-gray-900 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-start mb-7">
        <div>
          <h1 className="text-[1.875rem] font-bold text-gray-900 m-0 tracking-[-0.3px]">Payments</h1>
          <p className="text-[0.9rem] text-gray-500 mt-1 mb-0">Manage society maintenance and charges</p>
        </div>
      ...
      {activePayTab === "pending" && (
        <section>
          {pendingPayments.length > 0 && (
            <div className="py-2.5 px-3.5 bg-yellow-50 border border-yellow-200 rounded-lg mb-3.5 text-[0.875rem]">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-[0_1px_4px_rgba(0,0,0,0.05)] overflow-auto max-h-[520px]" onScroll={handleScroll}>
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 sticky top-0 z-[1]">
                  {["Description", "Amount", "Status", "Due Date", "Action"].map(h => (
                    <th key={h} className="py-3 px-4 text-left text-[0.75rem] font-bold text-gray-500 uppercase tracking-[0.4px] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
            ...
            {pendingPayments.filter(p => p.description.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
              <div className="text-center py-16 px-6">
                ...
                <p className="text-[0.85rem] text-gray-400">You're all caught up. Great job!</p>
              </div>
            )}

            {isFetchingMore && (
              <div className="text-center py-4 text-[0.85rem] text-blue-600 font-semibold">Fetching older records…</div>
            )}
            {!hasMore && pendingPayments.length > 0 && (
              <div className="text-center py-3 text-[0.78rem] text-gray-400">✓ All records loaded</div>
            )}
          </div>
        </section>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-[440px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-gray-200">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-[1.2rem] font-bold text-gray-900 m-0">New Payment</h2>
              <button onClick={() => setShowAdminForm(false)} className="bg-transparent border-none text-[1.1rem] text-gray-400 hover:text-gray-600 cursor-pointer p-1 leading-none">✕</button>
            </div>
            {[
            ...
            <div className="mb-4">
              <label className="block text-[0.78rem] font-semibold text-gray-700 mb-1.5 uppercase tracking-[0.3px]">Due Date</label>
              <input
                type="date"
                name="dueDate"
            ...
            <div className="flex justify-end gap-2.5 mt-6">
              <button onClick={() => setShowAdminForm(false)} className="py-2 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300 rounded-lg text-[0.875rem] font-semibold cursor-pointer transition-colors">Cancel</button>
              <button
                onClick={addPayment}
      ...
      {confirmModal.open && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-7 w-full max-w-[400px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-gray-200 text-center">
            ...
            <h2 className="text-[1.2rem] font-bold text-gray-900 m-0 mb-2 justify-center">Delete Payment?</h2>
            <p className="text-[0.875rem] text-gray-500 m-0 mb-1.5">Are you sure you want to delete</p>
            <p className="text-[0.9rem] font-semibold text-gray-900 m-0 mb-6">"{confirmModal.description}"</p>
            <p className="text-[0.8rem] text-gray-400 mb-6 -mt-3">This action cannot be undone.</p>
            <div className="flex gap-2.5 justify-center">
              <button
                onClick={() => setConfirmModal({ open: false, paymentId: null, description: "" })}
                className="flex-1 max-w-[140px] py-2.5 px-0 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300 rounded-lg text-[0.875rem] font-semibold cursor-pointer transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmDelete}
                className="flex-1 max-w-[140px] py-2.5 px-0 bg-red-600 hover:bg-red-700 text-white border-none rounded-lg text-[0.875rem] font-semibold cursor-pointer transition-colors"
              >
                Yes, Delete
              </button>
            </div>
