...
const getAllEvents = asyncHandler(async (req, res) => {
    const events = await Event.find({societyId : req.user.societyId}).populate("readyUsers" , "name phoneNo houseNo block -_id" ).select(" -updatedAt -__v -societyId -isActive -_id")
    // console.log(events)
    if(!events){
        return new ApiError( 500 ,"No events found" );
    }
